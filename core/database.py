...
def init_db():
    ...
    conn.executescript("""
        ...
        INSERT OR IGNORE INTO settings (key, value) VALUES ('show_app_images', '1');
...
def get_app_by_id(app_id: int) -> dict | None:
    rows = _fetch("SELECT * FROM tracked_apps WHERE id=?", (app_id,))
...
def update_tracked_app(app_id: int, **kwargs):
    allowed = {
        ...
        "img_icon", "img_banner", "img_poster",
    }
...
def import_profile_data(name: str, apps: list[dict], sessions: list[dict]) -> dict:
    """Create a new profile from exported JSON data. Returns summary."""
    # Ensure unique name
    existing = {p["name"] for p in get_profiles()}
    base, suffix = name, 1
    ...
    _q("INSERT INTO profiles (name) VALUES (?)", (name,))
    _commit()
    rows = _fetch("SELECT id FROM profiles WHERE name=?", (name,))
    profile_id = rows[0]["id"]

    # Insert apps, building old_id → new_id map
    app_id_map: dict[int, int] = {}
    for app in apps:
        exe = app.get("exe_name", "").lower().strip()
        if not exe:
            continue
        cur = _q("""
            INSERT OR IGNORE INTO tracked_apps
                (profile_id, exe_name, display_name, category, color, auto_detected,
                 cmdline_match, main_process_only, is_hidden, is_active,
                 img_icon, img_banner, img_poster)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            profile_id, exe,
            app.get("display_name", exe),
            app.get("category", "other"),
            app.get("color", "#6366f1"),
            app.get("auto_detected", 0),
            app.get("cmdline_match"),
            app.get("main_process_only", 0),
            app.get("is_hidden", 0),
            app.get("is_active", 1),
            app.get("img_icon"),
            app.get("img_banner"),
        ...
        if cur.lastrowid:
            app_id_map[app["id"]] = cur.lastrowid

    # Insert sessions mapped to new app IDs
    imported_sessions = 0
    for s in sessions:
        new_app_id = app_id_map.get(s.get("app_id"))
        if not new_app_id:
            continue
        _q("""
            INSERT INTO sessions (app_id, profile_id, started_at, ended_at, duration_secs, date)
            VALUES (?,?,?,?,?,?)
        """, (new_app_id, profile_id,
              s.get("started_at", ""), s.get("ended_at"),
              s.get("duration_secs", 0), s.get("date", "")))
        imported_sessions += 1

    _commit()
    return {"profile_id": profile_id, "name": name,
            "apps_imported": len(app_id_map), "sessions_imported": imported_sessions}
