...
class AppUpdate(BaseModel):
    ...
    img_icon:          Optional[str] = None
    img_banner:        Optional[str] = None
...
def add_app(body: AppAdd):
    ...
    tracker.reload()
    if db.get_setting("auto_fetch_images", "0") == "1":
        from core.image_fetcher import auto_fetch_background
        auto_fetch_background(app_id, body.display_name or body.exe_name, body.exe_name)
...
def update_app(app_id: int, body: AppUpdate):
    ...
    # Empty string for img fields means "clear" (set to NULL)
    for key in ("img_icon", "img_banner", "img_poster"):
        if key in data and data[key] == "":
            data[key] = None
    db.update_tracked_app(app_id, **data)
    tracker.reload()
    return {"ok": True}
...
_IMG_TYPES = {"icon", "banner", "poster"}
_IMG_EXTS  = {".jpg", ".jpeg", ".png", ".webp", ".gif", ".avif"}


# Specific routes must come BEFORE the generic /{img_type} route

@router.post("/apps/{app_id}/images/auto-fetch")
def auto_fetch_app_images(app_id: int):
    """Start artwork auto-fetch in background; returns immediately. Poll fetch-status for result."""
    from core.image_fetcher import auto_fetch_background
    app = db.get_app_by_id(app_id)
    if not app:
        raise HTTPException(404, "App not found")
    auto_fetch_background(app_id, app["display_name"], app["exe_name"])
    return {"ok": True, "started": True}


@router.get("/apps/{app_id}/images/fetch-status")
def app_fetch_status(app_id: int):
    """Poll background auto-fetch state for a specific app."""
    from core.image_fetcher import get_fetch_status
    return get_fetch_status(app_id)


@router.post("/apps/{app_id}/images/cancel")
def cancel_app_fetch(app_id: int):
    """Cancel any running fetch/search."""
    from core.image_fetcher import cancel_fetch
    cancel_fetch()
    return {"ok": True}


@router.post("/apps/{app_id}/images/icon/extract-exe")
def extract_icon_from_exe(
    app_id: int,
    exe_path: Optional[str] = Body(None, embed=True),
):
    """Extract the icon embedded in the app's .exe file (Windows only).
    Optionally accepts exe_path in JSON body; otherwise auto-detects."""
    from core.image_fetcher import find_exe_on_system, _extract_local_icon

    app = db.get_app_by_id(app_id)
    if not app:
        raise HTTPException(404, "App not found")

    resolved = exe_path if (exe_path and Path(exe_path).exists()) else find_exe_on_system(app["exe_name"])
    if not resolved:
        raise HTTPException(404, f"Executable not found for {app['exe_name']}")

    relative = _extract_local_icon(resolved, app_id)
    if not relative:
        raise HTTPException(500, "Icon extraction failed")

    db.update_tracked_app(app_id, img_icon=relative)
    return {"ok": True, "path": relative}


@router.post("/apps/{app_id}/images/{img_type}")
async def upload_app_image(app_id: int, img_type: str, file: UploadFile = File(...)):
    if img_type not in _IMG_TYPES:
        raise HTTPException(400, "img_type must be icon, banner, or poster")
    ext = Path(file.filename).suffix.lower() if file.filename else ".jpg"
    if ext not in _IMG_EXTS:
        ext = ".jpg"
    filename = f"{app_id}_{img_type}{ext}"
    for old in IMAGES_DIR.glob(f"{app_id}_{img_type}.*"):
        try: old.unlink()
        except Exception: pass
    (IMAGES_DIR / filename).write_bytes(await file.read())
    relative = f"app-images/{filename}"
    db.update_tracked_app(app_id, **{f"img_{img_type}": relative})
    return {"ok": True, "path": relative}


@router.delete("/apps/{app_id}/images/{img_type}")
def delete_app_image(app_id: int, img_type: str):
    if img_type not in _IMG_TYPES:
        raise HTTPException(400, "img_type must be icon, banner, or poster")
    app = db.get_app_by_id(app_id)
    if not app:
    ...
    if current and not current.startswith("http"):
        try: (IMAGES_DIR / Path(current).name).unlink(missing_ok=True)
        except Exception: pass
    db.update_tracked_app(app_id, **{col: None})
    return {"ok": True}


@router.post("/profiles/{profile_id}/images/auto-fetch-all")
def auto_fetch_all_profile_images(profile_id: int):
    """Lanza auto-fetch secuencial en background para todas las apps activas del perfil."""
    from core.image_fetcher import start_batch_fetch
    apps = db.get_tracked_apps(profile_id)
    active = [a for a in apps if a.get("is_active", 1)]
    start_batch_fetch(active)
    return {"ok": True, "queued": len(active)}
...
async def import_profile(file: UploadFile = File(...)):
    """Importa un perfil desde un JSON exportado. Crea un perfil nuevo, no reemplaza."""
...
class AppSettings(BaseModel):
    ...
    sgdb_api_key:      Optional[str]       = None
...
def get_app_settings():
    ...
    return {
        ...
        "show_app_images":   db.get_setting("show_app_images", "1") == "1",
        "auto_fetch_images": db.get_setting("auto_fetch_images", "0") == "1",
        "sgdb_api_key":      db.get_setting("sgdb_api_key", ""),
        "rawg_api_key":      db.get_setting("rawg_api_key", ""),
    }
...
def update_app_settings(body: AppSettings):
    ...
    if body.show_app_images is not None:
        db.set_setting("show_app_images", "1" if body.show_app_images else "0")
    if body.auto_fetch_images is not None:
        db.set_setting("auto_fetch_images", "1" if body.auto_fetch_images else "0")
    if body.sgdb_api_key is not None:
        db.set_setting("sgdb_api_key", body.sgdb_api_key.strip())
    if body.rawg_api_key is not None:
        db.set_setting("rawg_api_key", body.rawg_api_key.strip())
    return {"ok": True}
